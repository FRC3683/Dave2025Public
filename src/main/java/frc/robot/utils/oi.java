...
public final class oi {
    ...
    public static final double default_rumble = 0.25;
    ...
    public static void rumble_for(double seconds) { rumble_for(seconds, default_rumble).schedule(); }
    ...
    public static Supplier<Integer> create_axis(Supplier<Boolean> up, Supplier<Boolean> down) {
        return () -> {
            int x = 0;
            if(up.get()) {
                x += 1;
    ...
    public static Supplier<Integer> create_axis(Trigger up, Trigger down) {
        return () -> {
            int x = 0;
            if(up.getAsBoolean()) {
                x += 1;
    ...
    public static class shaping_chooser {
        ...
        public static double custom_old(double x) {
            if(x <= 0.5) {
                return 0.25 * x;
            }
        ...
        public static double custom(double x) {
            ...
            if (x <= l){
                ...
                h = (Math.pow(x, p) / (p*Math.pow(l, p-1)) + l - Math.pow(l, p) / (p*Math.pow(l, p-1)));
            }
            return h / (1 / (p*Math.pow(l, p-1)) + l - Math.pow(l, p) / (p*Math.pow(l, p-1)));
        }
