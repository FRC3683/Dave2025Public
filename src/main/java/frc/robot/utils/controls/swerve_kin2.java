...
public class swerve_kin2 extends SwerveDriveKinematics {
    ...
    public void to_module_states(ChassisSpeeds chassis_speeds, swerve_kin2_matrices matrices, module_state[] module_states) {
        if(chassis_speeds.vxMetersPerSecond == 0.0
                && chassis_speeds.vyMetersPerSecond == 0.0
                && chassis_speeds.omegaRadiansPerSecond == 0.0) {
            for (int i = 0; i < num_modules; i++) {
                module_states[i].speed_mps = 0.0;
            }
            return;
        }
        ...
        for (int i = 0; i < num_modules; i++) {
            ...
            module_states[i].speed_mps = speed;
            module_states[i].theta_rad = angle.getRadians();
            module_states[i].omega_radps = omega;
        }
    }
    ...
    public void to_module_states(ChassisSpeeds chassis_speeds, module_state[] module_states) {
        to_module_states(chassis_speeds, center_matrices, module_states);
    }
