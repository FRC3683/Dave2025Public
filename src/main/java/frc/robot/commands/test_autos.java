...
public class test_autos {
    ...
    public static Command straight(robot robot, double dist, double vel) {
        ...
        Commands.sequence(
            ...
            Commands.deadline(
                robot.swerve.strafe_line(new Translation2d(dist, 0), Rotation2d.fromDegrees(0), 4.3, 0.05, 0),
                robot.swerve.snap(0),
                new Command() {
                    double prev_speed = 0;
                    @Override
    ...
    public static Command through_test(robot robot, boolean through) {
        return Commands.sequence(
            robot.swerve.cmd_reset_pose(new Pose2d(2, 2, Rotation2d.fromDegrees(0))),
            robot.swerve.strafe_to_point(new Translation2d(4, 2), 4.3, 0.1, through ? 4.3 : 0),
            robot.swerve.strafe_to_point(new Translation2d(6, 4), 4.3, 0.05, 0)
        );
    }
    ...
    public static Command sttbolls_test(swerve swerve) {
        var snap_to = constants.tags.getTagPose(11).get().getRotation().toRotation2d().plus(Rotation2d.k180deg);
        return swerve.snap(() -> snap_to.getDegrees())
            .alongWith(swerve.strafe_sttbolls(config.LL_left, 11, 1.0));
    ...
    public static Command decel_test(swerve swerve) {
        return Commands.sequence(
            ...
            Commands.parallel(
                swerve.snap(0),
                swerve.strafe_to_point(new Translation2d(4.0, 0.0), max_speed_mps, 0, 0)
            )
        );
    }
}
